...
const MarcacaoIndex = () => {
  ...
  const [marcacaoToDelete, setMarcacaoToDelete] = useState(null);
        ...
        renderCell: (params) => (
          <>
            <Button
              ...
              onClick={() => nav("/marcacoes/edit/" + params.id)}
  ...
  const deleteMarcacao = (id) => {
    axios
      .delete(`http://localhost:5000/api/v1/marcacao/${id}`, {
        headers: {
      ...
      .then((res) => {
        if (res.status === 204) {
          ...
          setError("Ocorreu um erro ao eliminar a marcação.");
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/api/v1/marcacoes", {
        headers: {
          Authorization: token ? `Bearer ${token}` : undefined,
        },
      })
  ...
  return (
    <>
      ...
      <CustomModal
        ...
        text="Tem a certeza que pretende apagar a marcação?"
        buttonType="yesno"
        onYes={() => {
          deleteMarcacao(marcacaoToDelete);
          setModalOpen(false);
        }}
