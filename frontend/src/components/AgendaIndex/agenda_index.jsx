...
const AgendaIndex = () => {
        ...
        renderCell: (params) => (
          <>
            <Button
              ...
              onClick={() => nav("/agendas/edit/" + params.id)}
  ...
  const deleteAgenda = (id) => {
    axios
      .delete(`http://localhost:5000/api/v1/agenda/${id}`, {
        headers: {
      ...
      .then((res) => {
        if (res.status === 204) {
          ...
          setError("Ocorreu um erro ao eliminar a agenda.");
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/api/v1/agendas", {
        headers: {
          Authorization: token ? `Bearer ${token}` : undefined,
        },
      })
  ...
  return (
    <>
      ...
      <CustomModal
        ...
        text="Tem a certeza que pretende apagar a agenda?"
        buttonType="yesno"
        onYes={() => {
